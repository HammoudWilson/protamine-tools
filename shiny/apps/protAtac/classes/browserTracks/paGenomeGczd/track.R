...
build.paGenomeGczdTrack <- function(track, reference, coord, layout){
    ...
    gczd <- b[, 
        .(gczd = {
            weighted.mean(gcZScoreDelta(sourceId)$gczd[binI], basesInPixel, na.rm = TRUE) 
        }),
        keyby = .(pixel)
    ][, 
        gczd
    ...
    pngFile <- file.path(sessionDirectory, paste("paGenomeGczdTrack", "png", sep = "."))
...
items.paGenomeGczdTrack <- showTrackSourcesDialog
