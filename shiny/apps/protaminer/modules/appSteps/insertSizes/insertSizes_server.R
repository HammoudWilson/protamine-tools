...
#----------------------------------------------------------------------
getInsertSizeMatrix <- function(n_is_gc_, sampleNames){
    n_is_gc <- n_is_gc_[,,sampleNames[1]]
    if(length(sampleNames) > 1){
        for (i in 2:length(sampleNames)){
            n_is_gc <- n_is_gc + n_is_gc_[,,sampleNames[i]]
        }
    }
    n_is_gc
}
insertSizeMatrixPlot <- function(refType){
    plot <- staticPlotBoxServer(
        ...
        create = function() {
            ...
            req(sourceId, selectedSampleNames)
            dstr(selectedSampleNames)
            n_is_gc <- paCollateData_v5(sourceId)$n_is_gc_ref[[refType]]
            n_is_gc <- getInsertSizeMatrix(n_is_gc, selectedSampleNames)
...
getInsertSize_refType <- function(n_is_gc_ref, refType, samples){
    ...
    sapply(samples$sample_name, function(sample_name){
        c(
            ...
            rowSums(n_is_gc_ref[[refType]][,,sample_name])
        )
    }, simplify = FALSE, USE.NAMES = TRUE) %>% as.data.table
}
getInsertSizeData <- function(n_is_gc_ref, refType, samples, aggregate, normalize){
    isdt_this <- getInsertSize_refType(n_is_gc_ref, refType, samples)
    if(aggregate) isdt_this <- aggregateInsertSizes(isdt_this, samples)
    if(normalize){
        isdt_ref <-getInsertSize_refType(n_is_gc_ref, "spike_in", samples)
...
insertSizesPlot <- function(refType){
    plot <- staticPlotBoxServer(
        ...
        create = function() {
            ...
            n_is_gc_ref <- paCollateData_v5(sourceId)$n_is_gc_ref
            aggregate <- input$aggregateByStage
            normalize <- input$normalizeToSpikeIn
            if(refType == "spike_in" && normalize) req(FALSE)
            isdt <- getInsertSizeData(n_is_gc_ref, refType, selectedSamples, aggregate, normalize)
            ...
            for(series in seriesNames){
                plot$addLines( # addLines follows the same pattern, etc.
                    ...
                    y = isdt[[series]][MIN_INSERT_SIZE:MAX_INSERT_SIZE],
                    col = colors[series]
                )
            }
            plot$addLegend(
