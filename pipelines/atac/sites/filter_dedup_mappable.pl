...
# dependencies
require "$ENV{MODULES_DIR}/mappability/insert_size_levels.pl";
...
foreach my $insertSizeLevel(@MAPPABILITY_SIZE_LEVELS){
    ...
    my $intersect = "bedtools intersect -u -sorted -g $ENV{GENOME_FASTA_SHM}.fai -a - -b $mppFile | ".
                    "awk 'BEGIN{OFS=\"\\t\"}{print \$1, \$2, \$4, \$5}' > $islFile";
    open my $outH, "|-", $intersect or die "filter_dedup_mappable error: cannot open $islFile for streaming\n";
...
sub processStart0 {
    ...
    foreach my $f(@start0Group){
        ...
        print $outH join("\t", 
            ...
            $$f[END1],       # ... while retaining each original insert end (could be multiple distinct ends for each start)
            $insertSizeLevel
        ), "\n";
    }
}
...
open my $stageCountH, ">>", $ENV{STAGE_COUNT_FILE} or die "filter_dedup_mappable error: cannot open $ENV{STAGE_COUNT_FILE} for appending\n";
print $stageCountH join("\t", $ENV{SAMPLE_NAME}, "stage_3_dedup", $nPassed), "\n";
close $stageCountH;
