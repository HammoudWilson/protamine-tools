...
INPUT_FILE=${TASK_DIR}/${DATA_NAME}.mm39_dm6.gencode.M36.tss.bed.gz

# name and excerpt the input file(s) to the job log
echo "input file: ${INPUT_FILE}" 1>&2
zcat ${INPUT_FILE} | head 1>&2

# adjust to 100bp-padded TSS regions with log10 RPKM values, active TSS only
zcat ${INPUT_FILE} |
...
awk -v PADDING=${TSS_PADDING} -v M=${LOG10_M}  -v MIN_RPKM=${MIN_TSS_RPKM} -v MIN_FOLD=${MIN_TSS_FOLD_INC} 'BEGIN{
    OFS = "\t";
    print "chrom", "start0", "end1", "name", "Log10_TSS_RPKM";
} $8 >= MIN_RPKM && $9 >= MIN_FOLD {
