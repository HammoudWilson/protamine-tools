...
awk 'BEGIN{OFS="\t"}{ print $1, $2, $3, $4, $5, $6, ($10 == "." ? 0 : $10) }' | 
bedtools groupby -g 1,2,3,4,5,6 -c 7 -o mean | 
awk -v M=${LOG10_M} 'BEGIN{
    OFS = "\t";
    print "chrom", "start0", "end1", "name", "Log10_RPKM", "strand";
} {
    if($7 == "." || $7 < 0.001) {
        log10_rpkm = -3;
    } else {
        log10_rpkm = int(log($7)/log(10) * M + 0.5) / M;
    }
    print $1, $2, $3, $4, log10_rpkm, $6;
}'
